...
$robots = 'index, follow';
$extraHeadTags = "<link rel=\"preconnect\" href=\"https://fonts.googleapis.com\">\n" .
...
<section class="relative bg-[var(--warm-white)] border-b border-[var(--border)] pt-20 pb-24 px-6 overflow-hidden">
    ...
    <div class="max-w-4xl mx-auto text-center relative z-10">
        <div
            class="inline-flex items-center gap-2 bg-[var(--teal-light)] text-[var(--teal-dark)] px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mb-8">
            <span class="material-symbols-outlined text-base">verified</span>
            Verified by Registered Nurses
        </div>
        <h1
            class="text-5xl md:text-6xl lg:text-[4rem] font-serif font-bold leading-[1.1] text-[var(--ink)] mb-8 tracking-tight">
        ...
        <div
            class="mt-16 flex flex-wrap justify-center gap-8 md:gap-16 border-t border-[var(--border)] pt-8 max-w-3xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-[var(--warm-white)] p-8 rounded-2xl border border-[var(--border)] shadow-sm">
                ...
                <p class="text-[var(--ink-light)] leading-relaxed">
                    Every prompt is written and reviewed by registered nurses with clinical experience in that specific
                    field.
                </p>
            </div>
            <div class="bg-[var(--warm-white)] p-8 rounded-2xl border border-[var(--border)] shadow-sm">
            ...
            <div class="bg-[var(--warm-white)] p-8 rounded-2xl border border-[var(--border)] shadow-sm">
                ...
                <p class="text-[var(--ink-light)] leading-relaxed">
                    Designed for your perspective, addressing the fears, doubts, and daily challenges doctors often
                    miss.
                </p>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
            ...
            <div class="text-center relative">
                <div
                    class="w-24 h-24 bg-[var(--cream)] border-2 border-[var(--border)] rounded-full flex items-center justify-center text-3xl font-serif font-bold text-[var(--amber)] mx-auto mb-6 shadow-sm z-10">
                    1</div>
...
<section class="py-24 px-6 bg-[var(--cream)]" id="categories">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <?php $t = $category['tone']; ?>
            <a class="category-card bg-[var(--warm-white)] border border-[var(--border)] rounded-xl p-6 transition-all duration-300 group"
                href="<?= app_h((string)$category['link']); ?>">
                <div class="flex items-start justify-between mb-4">
                    <div
                        class="bg-[var(--<?= $t === 'amber' ? 'amber-pale' : 'teal-light'; ?>)] p-3 rounded-lg text-[var(--<?= $t === 'amber' ? 'amber-dark' : 'teal-dark'; ?>)]">
                        <span class="material-symbols-outlined text-2xl">
                            <?= app_h((string)$category['icon']); ?>
                        </span>
                    </div>
                    <span
                        class="text-xs font-bold bg-[var(--cream)] text-[var(--ink-muted)] px-2 py-1 rounded border border-[var(--border)]">
                        <?= app_h((string)$category['prompt_count']); ?>
                    </span>
                </div>
                <h3
                    class="font-serif text-xl font-bold text-[var(--ink)] mb-2 group-hover:text-[var(--amber)] transition-colors">
                    <?= app_h((string)$category['title']); ?>
                </h3>
                <p class="text-sm text-[var(--ink-light)] line-clamp-2">
                    <?= app_h((string)$category['description']); ?>
                </p>
            </a>
            <?php
endforeach; ?>
...
<section class="py-12 px-6 bg-[var(--cream)]">
    <div class="max-w-7xl mx-auto">
        <div
            class="bg-gradient-to-br from-[var(--ink)] to-[#2D2520] rounded-2xl p-8 md:p-16 text-center shadow-xl relative overflow-hidden">
            <div
                class="absolute top-0 left-0 w-full h-full opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
            </div>
            <div class="relative z-10">
                <h2 class="font-serif text-3xl md:text-5xl font-bold text-white mb-6">Unlock better health
                    conversations.</h2>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                    <a class="btn-secondary border-transparent shadow-lg" href="/auth/register">Monthly — $17/mo</a>
                    <a class="btn-primary shadow-lg border-[var(--amber-light)] relative" href="/auth/register">
                        Annual — $99/yr
                        <span
                            class="absolute -top-3 -right-3 bg-[var(--teal)] text-white text-[11px] uppercase tracking-wider font-bold px-2 py-1 rounded shadow-md">Save
                            50%</span>
                    </a>
                </div>
                <p class="text-white/40 text-sm font-medium mt-6">Create an account to subscribe. Cancel anytime.</p>
            </div>
        </div>
    </div>
</section>
